...
import { SectionCreation } from "../../section/components/SectionCreation";
...
const CourseManager = () => {
  ...
  const {
    ...
    error: fetchError,
  } = useApi(CourseServices.getCourseDetail);
        ...
        <div className="flex-none w-2/3 mr-20">
          {tickChange === 0 && (
            <CourseComponent
              ...
              setTickChange={handleTickChange}
              setId={setId}
              updateHighestTick={updateHighestTick}
            />
          )}
          {tickChange === 1 && (
            <SectionCreation setTickChange={handleTickChange} />
          )}
          {tickChange === 2 && (
            <CoursePreview id={id ?? ""} setTickChange={handleTickChange} />
          )}
        </div>
      </div>
    </Layout>
