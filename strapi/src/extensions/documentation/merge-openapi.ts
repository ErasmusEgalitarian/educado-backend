...
const mergeSwaggerDocumentation = (): void => {
    ...
    if (!fs.existsSync(newSpecPath)) {
        console.log("Info: strapi-spec.json not found. Skipping merge (will be called during generate-merged-spec).");
        return;
    }
    ...
    let newSpec: any;
    try {
        const rawData = fs.readFileSync(newSpecPath, 'utf-8');
        newSpec = JSON.parse(rawData);
    } catch (e: any) {
        console.log("Info: Invalid or incomplete strapi-spec.json. Skipping merge (will be called during generate-merged-spec).");
        return;
    }
    ...
    let fullDocs: any;
    try {
        fullDocs = JSON.parse(fs.readFileSync(fullDocsPath, 'utf-8'));
    } catch (e: any) {
        console.warn("Warning: Could not read full_documentation.json, proceeding without legacy schemas.");
        fullDocs = { components: { schemas: {} } };
    }
